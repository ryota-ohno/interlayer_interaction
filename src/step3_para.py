...
def init_process(args):
    ...
    def get_init_para_csv(auto_dir,monomer_name,params_dict):
        ...
        xyz=[]
        mask=detect_peaks(z_2dlist, filter_size=7, order=0.8).mask###このfilterとorderの調整　-Rcの極大を探す
    ...
    auto_csv_path = os.path.join(auto_dir,'step3-para.csv')
    ...
    df_init=pd.read_csv(os.path.join(auto_dir,'step3-para_init_params.csv'))
    df_init['status']='NotYet'
    df_init.to_csv(os.path.join(auto_dir,'step3-para_init_params.csv'),index=False)
...
def listen(args):
    ...
    auto_csv = os.path.join(auto_dir,'step3-para.csv')
