...
from make_8_twist import exec_gjf##makeも作り変える
from step3_twist_vdw import get_c_vec_vdw
from step3_twist_vdw import detect_peaks
...
def init_process(args):
    ...
    def get_init_para_csv(auto_dir,monomer_name,params_dict):
        ...
        A1= params_dict['A1']; A2 = params_dict['A2']; theta = params_dict['theta']
        init_params_csv = os.path.join(auto_dir, 'step3_twist_init_params.csv')
        init_para_list=[]
        z_2dlist=get_c_vec_vdw(monomer_name,A1,A2,a_,b_,theta)##z_2dlistでは後ろでRcの極小を得るために正負反転したものを出力
        ...
        xyz=[]
        ma=detect_peaks(z_2dlist, filter_size=7, order=0.8).mask###このfilterとorderの調整　-Rcの極大を探す
        ...
        if len(xyz)>0:
            for i in range(len(xyz)):
                init_para_list.append([a_,b_,theta,A1,A2,xyz[i][0],xyz[i][1],xyz[i][2],'NotYet'])


        df_init_params = pd.DataFrame(np.array(init_para_list),columns = ['a','b','theta','A1','A2','cx','cy','cz','status'])##いじる
        df_init_params.to_csv(init_params_csv,index=False)
    
    params_dict_twist={'a':7.2,'b':6.0,'A1':0,'A2':0,'theta':25}##layer_i.csv的なものを作ってそれから読み取るのもあり
    get_init_para_csv(auto_dir,monomer_name,params_dict_twist)

    auto_csv_path = os.path.join(auto_dir,'step3-twist.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','A1','A2','E','E_i0','E_ip1','E_ip2','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##Eの内訳
    else:
...
def get_params_dict(auto_dir, num_nodes, fixed_param_keys, opt_param_keys):##['a','b','theta','A1','A2'        'cx','cy','cz']を出力  
    """
    前提:
        step3-para_init_params.csvとstep3-para.csvがauto_dirの下にある
    """
    init_params_csv=os.path.join(auto_dir, 'step3-twist_init_params.csv')
    df_init_params = pd.read_csv(init_params_csv)
    ...
    #fixed_param_keys = ['a','b','theta','A1','A2']     opt_param_keys = ['cx','cy','cz']
